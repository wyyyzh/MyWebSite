...
export const site: SiteConfig = {
  ...
  intro: [
    ...
    '它的思路是把内容和展示彻底分开：所有文案都收在 `src/data/` 里，改文字不用碰组件代码，保存即热更新。3131 行源码、8 个页面、11 个组件，构建产物压缩后约 100KB。',
    '目前有 8 个路由可以跳转：项目列表、项目详情、简历、时间线、发版日志、README。卡片悬停会浮出预览，进详情页还能看到背景、方法、结果与复盘的完整记录。',
    '部署也是自动的：代码 push 到 GitHub，Actions 自动构建并发布到 GitHub Pages，没有后端要维护。这份 README 就是站点的一部分 —— 导航栏里那个 README 入口，和我仓库里的内容是同一套。',
  ],
  ...
  readme: [
    ...
    '- 内容与展示分离：所有文案都在 src/data/ 下，改数据不用碰组件',
    '- GitHub Actions 自动构建，发布到 GitHub Pages',
    '',
    '## 页面结构',
    '- / 首页：标语、栏目入口、提交热力图、自我介绍、精选项目',
    '- /projects 项目列表：支持标签筛选和关键词搜索',
    '- /projects/:slug 项目详情：封面、数据栏、正文段落、文件结构、侧边目录',
    '- /resume 简历：项目经历自动生成，可直接打印导出 PDF',
    '- /timeline 人生版本历史 · /release 站点发版日志 · /readme 本页内容',
    '',
    '## 技术选择',
    '进场动画仍然用自写的 useInView + CSS 过渡实现，没有引入第三方动画库。',
    '悬停预览卡也是手写的：Portal 渲染到 body，靠近视口边缘自动翻转方向，触屏设备自动关闭。',
